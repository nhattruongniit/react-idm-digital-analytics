...
const SHOW_CIRCLE_TILE_CHART = 'CHARTS/SHOW_CIRCLE_TILE_CHART';
const HIDE_CIRCLE_TILE_CHART = 'CHARTS/HIDE_CIRCLE_TILE_CHART';
const RESET_CIRCLE_TILE_CHART = 'CHARTS/RESET_CIRCLE_TILE_CHART';

export const showCircleTileChart = chartId => ({
  type: SHOW_CIRCLE_TILE_CHART,
  payload: { chartId }
})

export const hideCircleTileChart = chartId => ({
  type: HIDE_CIRCLE_TILE_CHART,
  payload: { chartId }
})

export const resetCircleTileChart = () => ({
  type: RESET_CIRCLE_TILE_CHART
})
...
export const deleteSelectedCharts = () => async (dispatch, getState) => {
  ...
  const viewId = getState().charts.charts.view.id;
  dispatch(showCircleTileChart(selectedChartIds));
  ...
  try {
    ...
    alert("can't delete charts")
  }
}

export const deleteCharts = (viewID, chartIds) => async dispatch => {
  dispatch(showCircleTileChart(chartIds));
  dispatch(clearSelectedCharts());
  try {
...
export const duplicateChart = chartId => async (dispatch, getState) => {
  ...
  if (chartItems.length >= 12) {
    ...
    dispatch(showCircleTileChart(chartId));
    dispatch(clearSelectedCharts());
    const viewId = getState().charts.charts.view.id;
...
export const duplicateSelectedChart = () => async (dispatch, getState) => {
  ...
  if (chartItems.length + selectedChartIds.length >= 12) {
    ...
    try {
      ...
      console.log('Unable to duplicate chart', e);
    }
  }
}
...
export default function reducer(state = {
  ...
  switch (action.type) {
    ...
    case SHOW_CIRCLE_TILE_CHART: {
      const { chartId } = action.payload;
      let newObj = {};
      if (chartId.length > 0) {
        chartId.forEach(id => newObj[id] = true)
      } else {
        newObj = state.circles[chartId] = true;
      }
      return {
        ...state,
    ...
    case HIDE_CIRCLE_TILE_CHART: {
      const { chartId } = action.payload;
      if (chartId.length > 0) {
        chartId.forEach(id => delete state.circles[id])
      } else {
        delete state.circles[chartId]
      }
      return {
        ...state,
