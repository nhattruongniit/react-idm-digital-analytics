  ...
  <Container>
    {variableGroups.map(({ simulationId, simulationName, variables }) => (
      <SimulationGroup key={simulationId}>
        <SimulationName><span className="bx-text-default">{simulationName}</span></SimulationName>
        <List>
          {variables.length === 0 && <span className="bx-text-default">No available variable</span>}
